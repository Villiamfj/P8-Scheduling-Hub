...
class Solar_power_device_demo(Prod_device):
    def __init__(self, filePath:str, inverter_key = "1BY6WEcLGh8j5v7") -> None:
        super().__init__()
        rawData = pd.read_csv(filePath)
        # filtering
        rawData = rawData[(rawData["SOURCE_KEY"] == inverter_key)]
        self.data = rawData["AC_POWER"]
        # fixing index
    ...
    def get_history(self) -> List[float|int]:
        ...
        if self.counter >= 96: # the dataset contains 96 steps per day
            return [i / 1000 for i in self.data[self.counter - 96: self.counter]]
        return [i / 1000 for i in self.data[0 : self.counter]]
